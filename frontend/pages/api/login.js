import Cookies from 'cookies'
import clientPromise from "../../ABI/mongodb";
const {createHash} = require('node:crypto');

export default async function handler(req, res) {
  const loginMsg = "Incorrect the name of your deal or password";
  if (req.method == "POST") {
    const username = req.body['username'];
    const guess = req.body['password'];
    const client = await clientPromise;
    const db = client.db("Users");
    const users = await db.collection("Profiles").find({ "Username": username }).toArray();
    if (users.length == 0) {
      res.redirect(`/home?loginMsg=${encodeURIComponent(loginMsg)}`);
      return;
    }
    const user = users[0];
    const guess_hash = createHash('sha256').update(guess).digest('hex');
    if (guess_hash == user.Password) {
      const cookies = new Cookies(req, res);
      cookies.set('username', username);
      res.redirect("/profile");
    } else {
      res.redirect(`/home?loginMsg=Incorrect the name of your deal or password`);  // Pass the loginMsg parameter
    }
  } else {
    res.redirect("/profile");
  }
}
